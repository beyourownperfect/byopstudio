...
export default function Modal({ open, onClose, title, children, footer, size = "md" }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative w-full ${sizeCls} card-2 max-h-[90vh] flex flex-col animate-modal-in`} onClick={(e) => e.stopPropagation()}>
        {title && (
        ...
        {footer && <div className="px-5 py-3 border-t-2 border-border flex items-center justify-end gap-2">{footer}</div>}
      </div>
    </div>
  );
}
